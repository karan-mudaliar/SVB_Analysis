...
transformations_on_excel <- function(data, bank_flag){
  ...
  if (date_str_f2 == "x4"){
    ...
    Dates <- sapply(dates$dates, function(x) as.Date(x, format = "x%m.%d.%Y"))
    print(dates)
  }
...
bank2 <- read_excel("C:/Users/sasre/OneDrive/Documents/GitHub/SVB_Analysis/Data/First_Republic_bank.xlsx", sheet="Liquidity and Funding")
bank2_df <- transformations_on_excel(bank2, 1)
...
IRR_data %>% filter(Bank == "Silicon Valley Bank") %>% ggplot(aes(x = Date, y = `Held to Maturity`, group = 1)) + geom_line() + geom_point()
...
```{r}
ggplot() +
  geom_point(aes(x = LF_tot_data$Date, y = LF_tot_data$`Short Term Non Core Funding`))
```
```{r}
bank1 <- read_excel("C:/Users/sasre/OneDrive/Documents/GitHub/SVB_Analysis/Data/Citizens_bank.xlsx", sheet="Liquidity and Funding - Totals")
bank1_df <- transforamtions_on_excel(bank1, 2)
