...
aggregate_bank_reports <- function(bank_flag, pg_flag, sheet){
  ...
  if (pg_flag == 1){
    pg_df <- transforamtions_on_excel(bank3, 0)
    names(pg_df)[2] <- "Bank"
...
LF_tot_data <- aggregate_bank_reports(1, 2, "Liquidity and Funding - Totals")
LI_data <- aggregate_bank_reports(1, 1, "Liquidity & Investment")
LI_tot_data <- aggregate_bank_reports(1, 2, "Liquidity & Investment - Totals")
CAD_data <- aggregate_bank_reports(1, 2, "Capital Analysis D")
```
